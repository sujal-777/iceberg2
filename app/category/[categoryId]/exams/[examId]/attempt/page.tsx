...
export default function MockTestPage() {
  ...
  const fetchPdfUrl = async () => {
    try {
      ...
      const response = await fetch(`https://icebreg-backend2.onrender.com/api/test-series/${testSeriesId}/pdf-url`)
...
handleStartExam();
  // end apply for test 
    const fetchQuestions = async () => {
      try {
        setLoading(true)
        const response = await fetch(`https://icebreg-backend2.onrender.com/api/test-series/${testSeriesId}/questions`)
  ...
  const fetchIndividualQuestion = async (questionId: string) => {
    try {
      const response = await fetch(`https://icebreg-backend2.onrender.com/api/test-series/${testSeriesId}/questions/${questionId}`)
      if (response.ok) {
