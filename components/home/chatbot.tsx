...
// export default function IcyChat({
//   setOpenChat,
// }: {
...
export default function IcyChat({
  ...
  const saveUserInfo = async (info: UserInfo) => {
    try {
      ...
      console.log(await response.json());
      if (!response.ok) {
        addAgentMessage("There was an error saving your information. Please try again.");
      }
    } catch (error) {
      console.error("API call error:", error);
      addAgentMessage("There was an error saving your information. Please try again.");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === "default") {
      ...
      if (nextIndex < extractionQuestions.length) {
        ...
        addAgentMessage("Thank you for providing your details. Saving your information...");
        const completeUserInfo = { ...userInfo, exam: answer };
        await saveUserInfo(completeUserInfo);
        addAgentMessage("Your information has been saved successfully!");
        addAgentMessage("You can now ask me questions about the company.");
      ...
      try {
        ...
        console.error(err);
        addAgentMessage("Sorry, I encountered an error while processing your question.");
      }
    }
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 animate-slide-in transition-all duration-300">
      <div className="w-[90vw] sm:w-[380px] h-[600px] max-h-[90vh] bg-white rounded-3xl shadow-2xl flex flex-col overflow-hidden border border-gray-200">
        {/* Header */}
        <div className="relative p-4 sm:p-6 bg-gradient-to-r from-[#4629f2] to-[#8b5cf6] text-white rounded-t-3xl">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full overflow-hidden">
              <Image src="/iceberg-chat.jpeg" alt="Icy logo" width={40} height={40} />
            </div>
            <h1 className="text-xl sm:text-2xl font-bold">Icy</h1>
          </div>
          ...
          <button
            onClick={handleClose}
            className="absolute top-3 right-3 sm:top-4 sm:right-4 w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-full bg-white/20"
          >
        ...
        {/* Chat Area */}
        <div
          className="flex-1 p-4 sm:p-6 bg-[#eef6ff] overflow-y-auto"
          ref={chatContainerRef}
        >
          {messages.map((msg, idx) => (
            <div
            ...
            >
              {msg.sender === "agent" && (
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full overflow-hidden flex-shrink-0">
                  <Image src="/iceberg-chat.jpeg" alt="Icy avatar" width={40} height={40} />
                </div>
              )}
              <div
                className={`p-3 sm:p-4 rounded-2xl max-w-[80%] text-sm sm:text-base ${
                  msg.sender === "agent"
                    ? "bg-blue-100 text-blue-900 ml-3"
                    : "bg-[#4629f2] text-white mr-3"
                }`}
        ...
        <form onSubmit={handleSubmit} className="p-3 sm:p-4 border-t border-gray-200">
          <div className="flex items-center gap-2 p-2 border border-gray-300 rounded-full bg-white">
            <input
              type="text"
              placeholder="Reply ..."
            ...
            <button
              type="submit"
              className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-[#4629f2] flex items-center justify-center"
            >
